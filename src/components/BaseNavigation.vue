  ...
  <ul :class="classContainer">
    ...
    >
      ...
      <router-link
        ...
        :to="node.to || '#'"
        class="nav-main-link"
        :active-class="(node.to && node.to !== '#' && node.to !== '/' ) ? 'active' : ''"
        @click.native="linkClicked($event)"
      >
        <i v-if="node.icon" :class="`nav-main-link-icon ${node.icon}`"></i>
        <span v-if="node.name" class="nav-main-link-name" :class="'class_'+node.name">{{ node.name }}</span>
    ...
    <!-- <li class="nav-main-item" style="position:fixed; bottom: 0px; right:0; left:0; background: #035432;">
      <a class="nav-main-link nav-main-link" id="logout" href="/?logout=true">
